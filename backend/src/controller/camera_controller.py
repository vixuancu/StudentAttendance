...
class CameraController:
    ...
    async def get_cameras(
        ...
        return ListResponse(
            data=[CameraDetailResponse.model_validate(camera) for camera in cameras],
            total=total,
    ...
    async def get_camera(self, id: int) -> DataResponse[CameraDetailResponse]:
        camera = await self.service.get_camera(id)

        return DataResponse(
            data=CameraDetailResponse.model_validate(camera),
            message="Lấy thông tin tài khoản thành công",
        )

    async def create_camera(
        self, request: CameraCreateRequest
    ) -> DataResponse[CameraDetailResponse]:
    ...
    async def delete_camera(self, id: int) -> DataResponse[CameraDetailResponse]:
