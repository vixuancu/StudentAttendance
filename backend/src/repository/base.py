...
)  # tạo kiểu tổng quát cho các model kế thừa từ Base
...
class BaseRepository:
    ...
    async def get_by_id(self, id: int) -> Optional[ModelType]:
        ...
        )  # trả về một bản ghi hoặc None nếu không tìm thấy

    async def get_all(
        self, skip: int = 0, limit: int = 100, filters: Optional[List[Any]] = None
    ) -> List[ModelType]:
    ...
    async def update(self, db_obj: ModelType, update_data: dict) -> ModelType:
        ...
        for field, value in update_data.items():
            if value is not None:
                setattr(
                    db_obj, field, value
                )  # cập nhật trường nếu giá trị không phải None
        await self.db.flush()
