...
class ReportService(IReportService):
    ...
    async def get_report_stats(
        ...
        data = await self.report_repo.get_report_stats(course_section_id, from_date, to_date)
    ...
    async def get_weekly_trend(
        ...
        data = await self.report_repo.get_weekly_trend(course_section_id, from_date, to_date)
    ...
    async def get_report_details(
        ...
        for record in records:
            ...
            session_date_str = session.session_date.strftime("%Y-%m-%d") if session.session_date else ""
            attendance_time = record.created_at.strftime("%H:%M:%S") if record.created_at else None
            
            detail_items.append(ReportDetailItem(
                id=record.id,
